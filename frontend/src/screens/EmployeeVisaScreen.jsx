...
import { useGetVisaStatusByIdQuery, useUpdateVisaStatusMutation } from '../slices/usersApiSlice';

export const EmployeeVisaManagement = () => {
  const [file, setFile] = useState(null);
  const { userInfo } = useSelector((state) => state.auth);
  const { data: visaStatus, isLoading, isError } = useGetVisaStatusByIdQuery(userInfo._id);
  const [updateVisaStatus, { isLoading: isUpdating }] = useUpdateVisaStatusMutation();
    ...
    switch (status.currentDocument) {
      case 'OPT Receipt':
        if (status.documents.optReceipt.status === 'Pending') {
          ...
          setMessage(`Your OPT Receipt was rejected. Feedback: ${status.documents.optReceipt.feedback}`);
      ...
      case 'OPT EAD':
        if (status.documents.optEAD.status === 'Pending') {
          ...
          setMessage(`Your OPT EAD was rejected. Feedback: ${status.documents.optEAD.feedback}`);
      ...
      case 'I-983':
        if (status.documents.i983.status === 'Pending') {
          ...
          setMessage(`Your I-983 was rejected. Feedback: ${status.documents.i983.feedback}`);
        } else if (status.documents.i983.status === 'Approved') {
          setMessage('Please send the I-983 along with all necessary documents to your school and upload the new I-20');
          setCurrentStep('I-20');
        }
        break;
      case 'I-20':
        if (status.documents.i20.status === 'Pending') {
          ...
          setMessage(`Your I-20 was rejected. Feedback: ${status.documents.i20.feedback}`);
      ...
      <ListGroup className='mt-3'>
        {Object.entries(documents).map(([docType, doc]) => (
          <ListGroup.Item key={docType}>
            ...
            {doc.file ? (
              <a href={doc.file} target="_blank" rel="noopener noreferrer">View Document</a>
            ) : (
              'Not uploaded'
            )}
            {' - '}
  ...
  return (
    <Container>
      ...
      {isAllDocumentsApproved() ? (
        ...
        <>
          {['OPT Receipt', 'OPT EAD', 'I-983', 'I-20'].includes(currentStep) && (
            <>
              ...
              <Button 
                ...
                disabled={!file || visaStatus.visaStatus.documents[mapDocumentType(currentStep)].status === 'Approved' || isUpdating}
              >
          ...
          {currentStep === 'I-983' && (
            <div className='mt-3'>
              <Button href="/path-to-empty-i983.pdf" download className="me-2">Download Empty I-983 Template</Button>
              <Button href="/path-to-sample-i983.pdf" download>Download Sample I-983 Template</Button>
            </div>
          )}
